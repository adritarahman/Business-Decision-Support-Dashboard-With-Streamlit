...
# product_df, sales_df,purchase_df=None,None,None
...
if products_df is not None:
    ...
    with revenue_col:
        st.metric(
            ...
            value=f"{key_matrices['Total Revenue (K)']}"
        )
    with profit_col:
        st.metric(
            label='Total Profit (K)',
            value=f"{key_matrices['Total Profit (K)']}"
        )

    with units_sold_col:
        st.metric(
            label='Total Units Sold (K)',
            value=f"{key_matrices['Total Units Sold (K)']}"
        )
    with low_stock_col:
        st.metric(
            label='Total Understocked Products',
            value=f"{key_matrices['Total Understocked Products']}"
        ) 
...
stock_issues['suggested_reorder'] = np.where(stock_issues['stock_status'] == 'Understocked',stock_issues['reorder_level'] - stock_issues['current_stock'], 0)                                     
